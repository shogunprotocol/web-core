        ...
        <div className={clsx("py-8 lg:py-16", className)} {...props}>
            <div className="relative bg-white/2.5 py-16 md:rounded-3xl lg:rounded-[3rem] xl:px-24 xl:py-32">
                ...
                <div className="absolute left-1/2 top-0 -translate-x-1/2 -translate-y-1/2 rounded-full bg-zinc-950 p-1.5">
                    <div className="h-1.5 w-8 rounded-lg bg-basement-cyan"></div>
                </div>
...
export function SectionBadge({ children, className }) {
    return <span className={clsx("text-sm tracking-tight text-basement-cyan uppercase font-basement", className)}>{children}</span>
}
...
export function SectionHeadingHighlighted({ children, className }) {
    return (
        <div>
            <div
                className={clsx(
                    "flex flex-col bg-[radial-gradient(35%_128px_at_50%_100%,theme(backgroundColor.basement-cyan/5%),transparent)] pb-8 md:items-center lg:pb-16",
                    className,
                )}>
                {children}
            </div>

            <div className="h-px w-full bg-gradient-to-r from-transparent via-basement-cyan/50 to-transparent"></div>
        </div>
    )
}
